...
void UpdatePlayer()
{
	...
	if (!pMouse->UseFlag)
	{
		...
		if (g_Player.UseFlag == true)
		{
			...
			//{
			//	THORN_BLOCK* thornblock = GetThornBlock();

			//	if ((thornblock + i)->UseFlag == true)
			//	{
			//		//プレイヤー左・ブロック右
			//		if (g_Player.Position.x + g_Player.size.x / 2 > (thornblock + i)->Position.x - (thornblock + i)->Size.x / 2 &&
			//			g_Player.oldpos.x + g_Player.size.x / 2 <= (thornblock + i)->Position.x - (thornblock + i)->Size.x / 2 &&
			//			g_Player.Position.y + g_Player.size.y / 2 > (thornblock + i)->Position.y - (thornblock + i)->Size.y / 2 &&
			//			g_Player.Position.y - g_Player.size.y / 2 < (thornblock + i)->Position.y + (thornblock + i)->Size.y / 2)
			//		{
			//			g_Player.Position.x = (block + i)->Position.x - (block + i)->Size.x / 2 - g_Player.size.x / 2;
			//		}
