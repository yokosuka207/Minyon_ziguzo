	...
	for (int i = 0; i < BLOCK_CHIP_ARRAY; i++) {
		...
		for (int j = 0; j < BLOCK_CHIP_ARRAY; j++) {
			...
			case static_cast <int> (MAPCHIP_TYPE::TYPE_BLOCK) :	//1
				SetBlock(D3DXVECTOR2((pos.x - BLOCK_CHIP_SIZE * BLOCK_CHIP_ARRAY / 2) + j * BLOCK_CHIP_SIZE + BLOCK_CHIP_SIZE / 2, (pos.y - BLOCK_CHIP_SIZE * BLOCK_CHIP_ARRAY / 2) + i * BLOCK_CHIP_SIZE + BLOCK_CHIP_SIZE / 2), D3DXVECTOR2(BLOCK_CHIP_SIZE, BLOCK_CHIP_SIZE));
				break;
